...
2. **UI/UX Research** → `ui-ux-researcher` evaluates UI needs, audits existing design system and Flutter widgets, identifies reuse opportunities to prevent duplication (optional, for UI-heavy features)
3. **Technical Research** → `researcher` analyzes current state, identifies constraints, explores implementation options, and recommends technical approach
4. **Product Requirements** → `product-owner` creates user stories, acceptance criteria, and defines scope
5. **Architecture** → `architect` designs system architecture, data flows, security, and telemetry
6. **Planning** → `tpm` (Technical Program Manager) creates implementation plan with task breakdown
7. **Test Specifications** → `ui-tester`, `flutter-tester`, `backend-tester` define test specs (TDD)
8. **Localization** → `i18n-localizer` extracts translatable strings, generates ARB files with context-aware translations in 6 languages, handles pluralization, gender, RTL, and cultural adaptation
9. **Implementation** → `ui-developer`, `flutter-developer`, `backend-developer` write code to repositories
10. **Code Review** → `ui-reviewer`, `flutter-reviewer`, `backend-reviewer` review code and identify issues
...
.agents/<feature>/
  ...
  research/
    component-analysis.md     # UI/UX Researcher output: UI requirements, existing components, duplication risks
    widget-mapping.md         # UI/UX Researcher output: Flutter widget reuse strategy
  ...
  i18n/
    strings.arb               # i18n-localizer output: Base English ARB
    translation-context.md    # i18n-localizer output: Context guide for translators
    locale-config.md          # i18n-localizer output: Locale settings
    translations/
      es.arb                  # Spanish translations
      fr.arb                  # French translations
      de.arb                  # German translations
      ar.arb                  # Arabic translations (RTL)
      ja.arb                  # Japanese translations
      zh.arb                  # Chinese (Simplified) translations
...
- **`ui-ux-researcher`** - Evaluates feature UI/UX needs, audits existing design system and Flutter widgets, identifies reuse opportunities to prevent duplication (optional, for UI-heavy features)
- **`researcher`** - Analyzes current state, identifies constraints, explores implementation options (≥3), and provides recommendation
- **`i18n-localizer`** - Extracts translatable strings, generates Flutter ARB files with context-aware translations in multiple languages (es, fr, de, ar, ja, zh), handles pluralization, gender, RTL, and cultural adaptation
...
```text
@agent-ideator: user-authentication
```

The ideator will transform your raw ideas into a structured `brief.md` with clear goals, scope, and requirements.

#### 2. UI/UX Research (Optional - for UI-heavy features)

```text
@agent-ui-ux-researcher: user-authentication
```

**Outputs:**
- `research/component-analysis.md` - UI requirements, existing components inventory, duplication risks
- `research/widget-mapping.md` - Flutter widget reuse strategy, composition patterns

> **Note:** Skip this step for backend-only features. Run this for features with UI components to prevent duplication and maximize widget reuse.

#### 3. Technical Research

```text
@agent-researcher: user-authentication
```
...
```text
@agent-product-owner: user-authentication
```
...
```text
@agent-architect: user-authentication
```
...
```text
@agent-ui-tester: user-authentication
@agent-flutter-tester: user-authentication
@agent-backend-tester: user-authentication
```
...
Extract and translate user-facing strings:

```text
@agent-i18n-localizer: user-authentication
```

**Outputs:**
- `i18n/strings.arb` - Base English ARB with metadata
- `i18n/translations/*.arb` - Translations for es, fr, de, ar, ja, zh
- `i18n/translation-context.md` - Context guide for translators
- `i18n/locale-config.md` - Locale settings and formatting rules
...
```text
@agent-ui-developer: user-authentication
@agent-flutter-developer: user-authentication
@agent-backend-developer: user-authentication
```
...
```text
@agent-ui-reviewer: user-authentication
@agent-flutter-reviewer: user-authentication
@agent-backend-reviewer: user-authentication
```
